...
USER_CONFIG_PATH="${HOME}/V22180_data/config"
MOONRAKER_CONFIG="${HOME}/V22180_data/config/moonraker.conf"
KLIPPER_PATH="${HOME}/klipper"

K_SHAKETUNE_PATH="${HOME}/V22180_data/klippain_shaketune"
K_SHAKETUNE_VENV_PATH="${HOME}/V22180_data/klippain_shaketune-env"
...
function preflight_checks {
    ...
    if [ "$(sudo systemctl list-units --full -all -t service --no-legend | grep -F 'klipper-V22180.service')" ]; then
...
function restart_klipper {
    ...
    sudo systemctl restart klipper-V22180
}

function restart_moonraker {
    echo "[POST-INSTALL] Restarting Moonraker..."
    sudo systemctl restart moonraker-V22180
}
